...
const NoteTaking = () => {
  ...
  const deletNote = (id) => {
      setAddItem((prev) => prev.filter((currData, indx) => {return indx !== id}))
      saveNote();
  }
  ...
  return (
    <>
      ...
      {addItem.map ((val, index) => {
        return (<div className="inline-block"><AddNote key={index} id={index} title={val.title} content={val.content} del={deletNote}/></div>)
      })}
