...
const DotGroup = ({selectedPage, setSelectedPage}) => {
  ...
  return (
    <div className='flex flex-col gap-6 fixed top-[60%] right-7'>
        ...
        <AnchorLink className={`${selectedPage === 'projects' ? selectedStyles : "bg-dark-grey"} w-3 h-3 rounded-full`} href='#projects' onClick={() => {setSelectedPage('projects')}}/>
        <AnchorLink className={`${selectedPage === 'testimonials' ? selectedStyles : "bg-dark-grey"} w-3 h-3 rounded-full`} href='#testimonials' onClick={() => {setSelectedPage('testimonials')}}/>
